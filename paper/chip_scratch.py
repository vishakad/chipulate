...
def findKlDistAll():
    ...
    tfsSelected = tfDf.query('maxEnergy >= 10')['tf']
    ...
    if os.path.isfile( 'data/klDistInfo.pickle'):
    ...
    for	tf in tfsSelected:
        ...
        baselineEnergies, baselineSequences, motifSequences, baselinePFM = getBaselineMotif(chemicalPotential=3,numLocations=numLocations,tf=tf,energiesSampled = energiesSampled)
    
        klDistInfo[tf] = {'normal':{},'powerLaw':{}}
        print( klDistInfo.keys() )
        for variationType in ['powerLaw','normal']:
                ...
                for mu in muList[variationType][variable]:
                    ...
                    for width in cvList:
                        ...
                        for rep in range(numReps):
                            genome = performChipSeq( baselineSequences, baselineEnergies, pExt=currentPext,
                            ...
                            pwmSingle, pcmSingle, pfmSingle = findPWM( learnGenome['sequence'].values )
